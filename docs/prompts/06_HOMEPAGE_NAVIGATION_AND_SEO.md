# 06 - HOMEPAGE NAVIGATION AND SEO
...
Build the localized public homepage and SEO metadata.

## Scope

Public pages and navigation only.

## Tasks

1. Build `/en` and `/he` homepage content for Studio Itai AI Coach Assistant.
2. Include short product explanation, new client CTA, existing user login link,
   language switcher, and theme toggle.
3. Use responsive design for mobile/tablet/desktop.
4. Add localized metadata for title, description, and Open Graph.
5. Add accessible landmarks and headings.
6. Ensure all copy is in translation files.
7. Ensure Hebrew layout is RTL and visually correct.

## Required Tests

1. Component or rendering test for homepage copy keys.
2. Playwright smoke tests for `/en` and `/he`.
3. Test CTA links preserve locale.
4. Verify metadata helper output if metadata generation uses helper functions.
...
```bash
git add .
git commit -m "Add localized homepage and SEO"
```
