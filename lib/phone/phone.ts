 ...
 * parentheses removed), and a single leading trunk-prefix 0 is dropped, before
 * the dial code is prepended. Returns an empty string when the national part
 * contains no digits, so an empty field stores as empty rather than a bare dial
 * code.
 *
 * Known limitation: trunk-prefix handling is country-specific, and this drops a
 * single leading 0 as the common case (correct for Israel and most countries).
 * A few countries that retain the leading 0 in E.164 (e.g. Italy fixed-line) are
 * not special-cased, a deliberate trade-off of not using a phone library.
 *
